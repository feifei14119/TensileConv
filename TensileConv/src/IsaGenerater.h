...
#define	c2hs(c)				std::string("0x0+"+c2s(c))
	...
	public:
		...
		~IsaGenerater() 
		{
			std::map<std::string, Var*>::iterator var;
			for (var = OperatorMap->begin(); var != OperatorMap->end(); var++)
			{
				delete var->second;
			}
			delete OperatorMap;
...
#pragma region ISA_REGION
		...
		E_ReturnState flat_load_dword_gfx800(int num, Var* v_dst, Var* v_offset_addr, int i_offset = 0, bool glc = false)
		{
			int tmpIdx;
			std::string str = "";
