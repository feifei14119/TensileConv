...
.GPR_ALLOC_BEGIN
    ...
    .LDS_ALLOC_FROM 0
	//.LDS_ALLOC lds_sig, SIG_NUM_PER_CU*4
.GPR_ALLOC_END
...
ConvFwd1x1:
    .amd_kernel_code_t	
		...
		float_mode 							= 240	// ?
		workgroup_group_segment_byte_size 	= .AUTO_LDS_BYTE_SIZE		//[caculated] group segment memory required by a work-group in bytes		
...
.macro ms_send_signal 			signal_type, index_reg
	.if (\signal_type == SIGNAL_REQ_FETCH)
		...
		s_mov_b32				s[s_tmp0], 1
		s_atomic_add			s[s_tmp0], s[s_ptr_sig:s_ptr_sig+1], s[s_tmp1]
	.elseif (\signal_type == SIGNAL_EXIT)
		s_mov_b32				s[s_signal], 1
		s_atomic_add			s[s_signal], s[s_ptr_sig:s_ptr_sig+1], 0
	.endif
.endm
...
/* 等待信号 																		*/
/************************************************************************************/
.macro m_wait_signal0			index_reg
...
FETCH_WAIT:	
	...
	s_cmp_eq_u32				s[s_signal], 0								// if(signal == SIGNAL_NULL) wait
	s_cbranch_scc1				FETCH_WAIT
...
FETCH_WAIT:	
	...
	s_load_dword				s[s_signal], s[s_ptr_sig:s_ptr_sig+1], 4		glc
	s_waitcnt 					lgkmcnt(0)
...
// ;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;	
	...
	// cu id
	s_lshr_b32					s[s_tmp1], s[s_tmp0], 8
	s_and_b32					s[s_tmp1], s[s_tmp1], 0xF
	// se id
	s_lshr_b32					s[s_tmp3], s[s_tmp0], 13
	s_and_b32					s[s_tmp3], s[s_tmp3], 0x3
	...
	//s_and_b32					s[s_weia0], s[s_tmp2+1], 0x01	
	s_mov_b32					s[s_weia0], s[s_tmp2+1]
	s_cmp_eq_u32				s[s_weia0], 0
	...
	s_ptr_wei_save0 = s_ptr_in
	s_mov_b64					s[s_ptr_wei_save0:s_ptr_wei_save0+1], s[s_ptr_wei:s_ptr_wei+1]
	
	// -------------------------------------------------------------------------------
	// PING_PANG_CU
	// -------------------------------------------------------------------------------
	s_and_b32					s[s_tmp1], s[s_tmp2+1], 0x01
...
FETCH_GROUP:
	// -------------------------------------------------------------------------------
	// 计算 signal 地址 
	// uint glb_sig_off = raw_id * SIG_NUM_PER_CU
	// -------------------------------------------------------------------------------
	s_lshl_b32					s[s_tmp0], s[s_tmp2+1], 0x0 + SIG_NUM_PER_CU_LOG2 + 0x2		// (+2:dword)
...
//	v_lshlrev_b32			v[v_addr_ds], 2, v[v_addr_ds]
	...
	s_store_dword				s[s_tmp1], s[s_ptr_sig:s_ptr_sig+1], 0 
	s_waitcnt					0
	s_mov_b32 					s[s_period_cnt], 4
...
CALCU_GROUP:
	...
	s_cmp_eq_u32				s[s_tmp2+1], 0
	s_cbranch_scc0				END_PROG
	//s_cmp_eq_u32				s[s_tmp2], 3
	//s_cbranch_scc0				END_PROG
...
//		s_store_dword			s[s_tmp2], s[s_ptr_sig:s_ptr_sig+1], s[s_tmp1]		
	m_load_input 				v_data0, enable
...
//	
//	// wave id
//	s_lshr_b32					s[s_tmp1], s[s_tmp0], 0				
//	s_and_b32					s[s_tmp1], s[s_tmp1], 0xF 			
//	s_store_dword				s[s_tmp1], s[s_ptr_sig:s_ptr_sig+1], 4*0
//	// simd id
//	s_lshr_b32					s[s_tmp1], s[s_tmp0], 4
//	s_and_b32					s[s_tmp1], s[s_tmp1], 0x3
//	s_store_dword				s[s_tmp1], s[s_ptr_sig:s_ptr_sig+1], 4*1
//	// pipe id
//	s_lshr_b32					s[s_tmp1], s[s_tmp0], 6
//	s_and_b32					s[s_tmp1], s[s_tmp1], 0x3
//	s_store_dword				s[s_tmp1], s[s_ptr_sig:s_ptr_sig+1], 4*2
//	// cu id
//	s_lshr_b32					s[s_tmp1], s[s_tmp0], 8
//	s_and_b32					s[s_tmp1], s[s_tmp1], 0xF
//	s_store_dword				s[s_tmp1], s[s_ptr_sig:s_ptr_sig+1], 4*3
//	// sh id
//	s_lshr_b32					s[s_tmp1], s[s_tmp0], 12
//	s_and_b32					s[s_tmp1], s[s_tmp1], 0x1
//	s_store_dword				s[s_tmp1], s[s_ptr_sig:s_ptr_sig+1], 4*4
//	// se id
//	s_lshr_b32					s[s_tmp1], s[s_tmp0], 13
//	s_and_b32					s[s_tmp1], s[s_tmp1], 0x3
//	s_store_dword				s[s_tmp1], s[s_ptr_sig:s_ptr_sig+1], 4*5
//	// group id
//	s_lshr_b32					s[s_tmp1], s[s_tmp0], 16
//	s_and_b32					s[s_tmp1], s[s_tmp1], 0xF
//	s_store_dword				s[s_tmp1], s[s_ptr_sig:s_ptr_sig+1], 4*6
//	// vm id
//	s_lshr_b32					s[s_tmp1], s[s_tmp0], 20
//	s_and_b32					s[s_tmp1], s[s_tmp1], 0xF
//	s_store_dword				s[s_tmp1], s[s_ptr_sig:s_ptr_sig+1], 4*7
//	
//	
//	s_waitcnt					0
//	s_dcache_wb
