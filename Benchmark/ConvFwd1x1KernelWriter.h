	...
	protected:
		...
		void init_output()
		{
			...
			if (c_in_group > 1)
			{
				...
				op2("v_mov_b32", v_addr_tmp, v_addr_out);
				op2("v_mov_b32", *v_addr_tmp + 1, *v_addr_out + 1);

				for (int i = 0; i < k_out_maps; i++)
				{
					flat_store_dword(1, v_addr_tmp, *v_acc_buff + i, "off");
					op4("v_add_co_u32", v_addr_tmp, "vcc", out_chan_stride * 4, v_addr_tmp);
